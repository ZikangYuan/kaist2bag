...
int EncoderConverter::Convert() {
    ...
    geometry_msgs::TwistStamped joint_data;
    ...
    while (fscanf(fp, "%ld,%ld,%ld\n", &stamp, &left_count, &right_count) == 3) {
        ...
        if(has_previous) {
            ...
            joint_data.header.stamp.fromNSec(stamp);
            joint_data.header.frame_id = "encoder";
            joint_data.twist.linear.x = wl * (double)encoder_left_diameter_ / 2;
            joint_data.twist.linear.y = wr * (double)encoder_right_diameter_ / 2;
